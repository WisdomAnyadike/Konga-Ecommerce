...
fetchedData.forEach((data, i) => {
  allItems.innerHTML += ` <div class="d-flex mb-4 align-items-center bg-light flex-column" style="width:fit-content; height:fir-content;" > <button id="${
    data.id
  }" value='${
    data.price
...
<b onclick="addToCart(event)" id="${data.id}" value='${
    data.price
  }' onmouseenter="changeColor(event)"  style="font-size:14px;height:30px; width:250px; border:1px solid #f855a6; color:#f855a6;" class="d-flex align-items-center justify-content-center "> Add to cart </b> </div>
